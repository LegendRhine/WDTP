			...
			while (i!=end) {
				if ((*i)->isBlankLine()) {
					...
					if (ii==end) {
						...
						const std::string& line_(*(*ii)->text());
						if (boost::regex_match(line_, m, continuationExpression)) {
...
optional<TokenPtr> parseListBlock(CTokenGroupIter& i, CTokenGroupIter end, bool sub=false) {
	...
	if (!(*i)->isBlankLine() && (*i)->text() && (*i)->canContainMarkup()) {
		...
		if (type!=cNone) {
			...
			size_t itemCount=1;
			std::ostringstream sub_;
			sub_ << "^" << std::string(indent, ' ') << " +(([*+-])|([0-9]+\\.)) +.*$";
			startSublistExpression=sub_.str();
			...
			while (i!=end) {
				if ((*i)->isBlankLine()) {
					...
					if (ii==end) {
						...
						const std::string& line_(*(*ii)->text());
						if (boost::regex_match(line_, startSublistExpression)) {
							...
							continue;
						} else if (boost::regex_match(line_, m, nextItemExpression)) {
							setParagraphMode=true;
							i=ii;
							nextItem=cAnotherItem;
						} else if (boost::regex_match(line_, m, continuedAfterBlankLineExpression)) {
							assert(m[1].matched);
							...
							i=++ii;
							continue;
						} else if (boost::regex_match(line_, m, codeBlockAfterBlankLineExpression)) {
							setParagraphMode=true;
							++itemCount;
							assert(m[1].matched);
							subItemTokens.push_back(TokenPtr(new markdown::token::BlankLine()));
							...
							while (ii!=end) {
								if ((*ii)->isBlankLine()) {
									...
									const std::string& line_2(*(*ii)->text());
									if (boost::regex_match(line_2, m, codeBlockAfterBlankLineExpression)) {
										codeBlock+=m[1]+'\n';
									} else break;
								} else break;
								++ii;
					...
					if (boost::regex_match(line_, startSublistExpression)) {
						...
						if (boost::regex_match(line_, m, cUnorderedListExpression)
							|| boost::regex_match(line_, m, cOrderedListExpression))
						{
							// Belongs to the parent list
							nextItem=cEndOfList;
						} else {
							boost::regex_match(line_, m, cContinuedItemExpression);
							assert(m[1].matched);
							subItemTokens.push_back(TokenPtr(new markdown::token::RawText(m[1])));
							++i;
							continue;
...
optional<TokenPtr> parseHeader(CTokenGroupIter& i, CTokenGroupIter end) {
	if (!(*i)->isBlankLine() && (*i)->text() && (*i)->canContainMarkup()) {
		...
		if (ii!=end && !(*ii)->isBlankLine() && (*ii)->text() && (*ii)->canContainMarkup()) {
			...
			const std::string& line_=*(*ii)->text();
			if (boost::regex_match(line_, m, cUnderlinedHeaders)) {
